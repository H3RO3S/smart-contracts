...
contract H3RO3S is ERC20PresetMinterPauser,Ownable {
    ...
    mapping(address => uint8) public walletType; 
    ...
    //To set Cliff period In Months 
    
    function setCliffPeriods(address[] memory _walletType,uint8[] memory cliffPeriod) public onlyOwner {
        require(_walletType.length == cliffPeriod.length,"Array Lengths not equal");
        for(uint256 i=0;i<=_walletType.length-1;i++) {
            require(cliffPeriod[i] <=12 && cliffPeriod[i] >0 );
            cliffPeriods[_walletType[i]] = cliffPeriod[i]*oneMonthinSeconds;
        }
        
    }
    
    // Input two array of intergers,first is wallet type and second is thier respective rate for buying during SaleType
    
    function setWalletBuyRate(uint256[] memory walletTypes,uint256[] memory rates) public onlyOwner {
        require(walletTypes.length == rates.length,"Array Lengths not equal");
        for(uint256 i=0;i<=walletTypes.length-1;i++){
            walletRates[walletTypes[i]]=rates[i];
        }
    }
    // To set Wallet Types foreach kind of vesting and SaleType
    
    function setWalletType(address[] memory _walleT,uint8[] memory type_) public onlyOwner {
        require(_walleT.length == type_.length,"Array lenghts not equal");
        for(uint256 i=0;i<=_walleT.length-1;i++)      {
            walletType[_walleT[i]] = type_[i];
        }
    }
    
    
    // To set Time Period of cliff, for share holder to be able to withdraw funds only after cliff period, if %=7.9 input 79
    
    function setUnlockPercents(uint8[] memory _walleTtype,uint256[] memory percent) public onlyOwner{
        require(_walleTtype.length == percent.length,"Array Lengths not equal");
        for(uint256 i=0;i<=_walleTtype.length-1;i++) {
            require(percent[i] >0 && percent[i] <100);
            unlockPercents[_walleTtype[i]]=percent[i];
        }
    }
    
    // To set Limits on amount of tokens use for different kind of sales and vestings
    function assignLimits(address[] memory _wallet,uint256[] memory limit) public onlyOwner{
        require(_wallet.length == limit.length,"Array length not equal");
        for(uint256 i=0;i<=_wallet.length-1;i++) {
            limits[_wallet[i]] = limit[i];
        }
    }
    ...
    function claimVestedTokens() public {
        ...
        uint256 amountToCLaim = limits[msg.sender]*ceilDiv(unlockPercents[_WType],1000);
        limits[msg.sender] -= amountToCLaim;
